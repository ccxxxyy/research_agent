...
本服务在其上叠加 **结构化情感评分 + 高频话题提取 + 多源融合**，
输出可量化、可审计的个股舆情报告。

数据源
------
1. **东方财富个股新闻** — 标题 + 摘要，逐条打分。
2. **雪球讨论热度榜** — 标的讨论量排名（stock_hot_tweet_xq），
   如果目标股票出现在榜上，融合其讨论量作为"热度信号"。
3. **东方财富热搜关键词** — 个股关联的热搜词（stock_hot_keyword_em），
   作为"市场关注点"直接呈现。
...
_POSITIVE_KEYWORDS: dict[str, float] = {
    # ── 业绩类 ──
    "业绩预增": 0.25, "业绩大增": 0.28, "超预期": 0.22,
    "净利润增长": 0.20, "净利润增": 0.20, "净利润上涨": 0.25,
    "净利上涨": 0.25, "净利润为": 0.10, "营收增长": 0.18,
    "营收增": 0.15, "扭亏为盈": 0.28, "盈利": 0.10,
    "利润增": 0.12, "同比增长": 0.12, "同比增": 0.10,
    "环比增长": 0.10, "环比增": 0.08, "创新高": 0.15,
    "历史新高": 0.18, "上涨": 0.08, "增速": 0.08,
    "高增长": 0.18, "翻倍": 0.22, "预喜": 0.18,
    "大幅增长": 0.22, "快速增长": 0.15, "稳步增长": 0.10,
    "复合增长": 0.12, "毛利率提升": 0.12, "净利率提升": 0.12,
    # ── 市场类 ──
    "涨停": 0.18, "大涨": 0.15, "突破": 0.08, "放量上涨": 0.15,
    "主力净流入": 0.12, "北向资金": 0.06, "增持": 0.15,
    "回购": 0.15, "分红": 0.12, "派息": 0.10, "送转": 0.08,
    "抢筹": 0.12, "净流入": 0.10, "连阳": 0.10,
    "新高": 0.12, "反弹": 0.06, "底部放量": 0.08,
    "强势": 0.08, "领涨": 0.10, "拉升": 0.08,
    # ── 政策/行业 ──
    "利好": 0.18, "政策支持": 0.15, "补贴": 0.12, "获批": 0.12,
    "中标": 0.15, "签约": 0.10, "合作": 0.05, "战略投资": 0.12,
    "产能扩张": 0.10, "订单": 0.10, "新产品": 0.10,
    "技术突破": 0.15, "募资": 0.06, "投资": 0.04,
    "成立": 0.03, "落地": 0.06, "量产": 0.10,
    "出海": 0.08, "全球化": 0.06, "国产替代": 0.12,
    "自主可控": 0.10, "降本增效": 0.10, "市占率提升": 0.12,
}

_NEGATIVE_KEYWORDS: dict[str, float] = {
    # ── 业绩类 ──
    "业绩预减": -0.22, "业绩下滑": -0.22, "业绩变脸": -0.25,
    "亏损": -0.20, "净利润下降": -0.18, "净利润下滑": -0.18,
    "净利润下": -0.15, "营收下降": -0.15, "营收下滑": -0.15,
    "同比下降": -0.12, "同比下滑": -0.10, "环比下降": -0.10,
    "毛利率下滑": -0.15, "计提减值": -0.18, "商誉减值": -0.20,
    "大幅下降": -0.22, "持续亏损": -0.22, "增收不增利": -0.12,
    # ── 市场类 ──
    "跌停": -0.18, "大跌": -0.15, "闪崩": -0.22, "暴跌": -0.25,
    "主力净流出": -0.12, "减持": -0.18, "高管减持": -0.22,
    "质押": -0.12, "爆仓": -0.25, "破位": -0.10,
    "缩量下跌": -0.12, "阴跌": -0.08, "套牢": -0.10,
    "割肉": -0.08, "破发": -0.12, "连跌": -0.10,
    # ── 风险/事件 ──
    "利空": -0.18, "处罚": -0.20, "立案调查": -0.28,
    "违规": -0.20, "退市": -0.28, "ST": -0.22, "*ST": -0.28,
    "风险提示": -0.15, "诉讼": -0.12, "被告": -0.15,
    "暂停上市": -0.25, "停牌": -0.10, "监管": -0.10,
    "警告": -0.15, "罚款": -0.18, "约谈": -0.12,
    "洗钱": -0.25, "失联": -0.22, "跑路": -0.28,
    "造假": -0.28, "虚增": -0.25, "起诉": -0.15,
    "被起诉": -0.18, "公诉": -0.20, "做空": -0.15,
    "谣言": -0.05, "扰动": -0.05,
}
...
_STOPWORDS: frozenset[str] = frozenset(
    ...
    "没有 看 好 自己 这 他 她 它 们 那 被 从 把 让 用 对 为 与 及 等 但 而 "
    "或 之 其 且 因 如 所 能 更 将 已 由 于 可 以 中 个 年 月 日 时 分 万 亿 "
    "元 股 公司 发布 公告 表示 记者 截至 目前 同时 以及 相关 根据 显示 数据 "
    "通过 进行 报告 方面 市场 行业 中国 以来 来源 编辑 责任 证券 财经 新闻 "
    "消息 据悉 获悉 上述 下述 其中 此外 此前 针对 关于 这是 该股 SZ SH".split()
)
...
def _aggregate_scores(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "overall_label": overall, "overall_score": round(avg, 4),
...
async def analyze_text_sentiment(texts: list[str]) -> dict:
    ...
    if not texts:
        return {
            "model_version": _MODEL_VERSION, "items": [],
...
async def get_stock_sentiment_report(symbol: str, limit: int = 30) -> dict:
    ...
    try:
        ...
        return _fmt_error(
            e, context=f"get_stock_sentiment_report(symbol={symbol!r}, limit={limit})",
        )
