...
- **MIXED**（跨市场）：必须遵守上下文中的 ``[MixedOrchestration]`` 清单。
  1. 按清单顺序，把 A 股子问题只交给 A 股侧专家，把美股子问题只交给美股侧专家；
  2. **禁止**用 A 股工具查美股标的，也禁止用美股工具查 A 股标的；
  3. 某一侧专家未挂载时，如实说明能力缺口，不要用另一侧工具硬查；
  4. 最终回答先分侧陈述关键数字，再做综合/对比；对比时标明币种、交易时区与会计口径差异。
  5. 共享专家（coder / knowledge）可在分侧取数之后用于计算或私有文档检索。
