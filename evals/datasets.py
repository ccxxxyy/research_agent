...
MIXED_ROUTING_PATH = _DATASETS_DIR / "mixed_market_routing.json"
...
def load_merged_routing_dataset(
    *,
    ...
    默认三者都加载；``limit`` 截断合并后的列表（顺序：CN → US → MIXED）。
    """
    ...
    if include_mixed and MIXED_ROUTING_PATH.is_file():
        examples.extend(load_json_dataset(MIXED_ROUTING_PATH))
